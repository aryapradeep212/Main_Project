...
const NavBar = () => {
  ...
  const handleLogout = () => {
    // Handle logout here (clear local storage, redirect, etc.)
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      ...
      <Drawer variant="permanent" open={open}>
        ...
        <Divider />
        <List>
          {[{text: 'User Details', path: '/user'},{text: 'Company Details', path: '/company'}, {text: 'Add Company', path: '/addcompany'},{text: 'Alumni Details', path: '/alumini'},{text: 'Add Alumni Details', path: '/addalumini'}, {text: 'Faculty Details', path: '/faculty'},{text: 'Add Faculty', path: '/addfaculty'},].map((item, index) => (
            <ListItem key={item.text} disablePadding sx={{ display: 'block' }}>
              ...
              >
                ...
                >
                  {index === 0 ? <VerifiedUserIcon />: index === 1 ? <BusinessIcon /> : index===2 ? <DomainAddIcon/> : index === 3 ? <SupervisorAccountIcon /> : index === 4 ? <PersonAddAltIcon/> : index===5 ? <PersonIcon /> : <PersonAddIcon/>}
                </ListItemIcon>
