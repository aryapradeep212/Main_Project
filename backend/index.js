...
// MongoDB Connection
mongoose.connect(
  "mongodb+srv://aryapradeep212:Arya@cluster0.zbd2evl.mongodb.net/?retryWrites=true&w=majority",
);
...
app.post("/addcompany", async (req, res) => {
  try {
    ...
    if (error.code === 11000) {
      res
        .status(400)
        .send("Duplicate key error. Company with same details already exists.");
    } else {
      res.status(500).send("An error occurred while saving the data.");
    }
  }
